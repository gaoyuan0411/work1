...
-- 资源点赞数据：dws__ri__area_like_stat__dd__full_v2
    ...
    ,area_pv AS (
             ...
             WHERE bd = '${biz_date}'
               ...
               AND app_type ='all'
               AND province_id IN
                   ('594033008543', '594033008593', '594033008573', '594033008595', '594033008585', '594033008591',
                    '594033008597')
        and city_id='all'
        and language='all'
    ...
    ,resours_pv AS (
        ...
        from (
            ...
            and azc.channel_code = dvcg.channel_code
                 left join (
            SELECT code       as channel_code,
                   name       as flag,
                   dict_order as order_num
            FROM nddc.dict_code
            where type = 'channels_v3'
            order by order_num asc
        ) as dctt on azc.channel_code = dctt.channel_code
...
-- 群组数据（师生群+家校群）
    --班均群组情况
    ,group_activity_tmp as (
        select *
    FROM nddc.dwd__tala__group_activity__d__full
    WHERE dt = '${biz_date}'
)
   ,activity_submit_tmp as (
       select * from nddc.dwd__tala__user_activity_submit__d__full where dt='${biz_date}'
)
   ,activity_read_tmp as (
       select * from nddc.dwd__tala__user_activity_read__d__full where dt='${biz_date}'
)
    ,group_data_count as (
    ...
    ,publish_activity_user AS (
        ...
        FROM group_activity_tmp
        WHERE  activity_publish_date <= '${biz_date}'
...
--   2024-05-18 调整增加*（该地区注册师生数/该地区学生教师基数）
    ,account_tmp  AS (
        select * from nddc.dwd__md__person_account__d__full where dt = '${biz_date}'
)
                 ...
                 from (
                          ...
                          from (
                              ...
                              from nation_province_data t3
                                       inner join group_activity_tmp t2
                                                  on t2.group_province_id = t3.real_province_id
                                       inner join activity_read_tmp t1
                                                  on t1.activity_id = t2.activity_id and t1.activity_type = t2.activity_type
                              where  t1.user_is_student = 1
                                AND t2.x_product_code='zxx'
                                AND t1.x_product_code='zxx'
...
, final_all_data as (
    select tmp.province_id,
           max(tmp.teacher_register_score) as teacher_register_score,
           max(tmp.student_register_score) as student_register_score,
           max(tmp.guardian_register_score) as guardian_register_score,
           max(tmp.area_activation_score) as area_activation_score,
           max(tmp.school_activation_score) as school_activation_score,
           max(tmp.teacher_verify_score) as teacher_verify_score,
           max(tmp.area_manager_active_score) as area_manager_active_score,
           max(tmp.area_pv_score) as area_pv_score,
           max(tmp.register_pv_score) as register_pv_score,
           max(tmp.resource_pv_score) as resource_pv_score,
           max(tmp.resource_assessment_score) as resource_assessment_score,
           max(tmp.class_group_score) as class_group_score,
           max(tmp.user_participate_score) as user_participate_score,
           max(tmp.activity_publish_score) as activity_publish_score,
           max(tmp.summer_special_activity_2022_score) as summer_special_activity_2022_score,
           max(tmp.summer_special_activity_2023_score) as summer_special_activity_2023_score,
           max(tmp.winter_special_activity_2023_score) as winter_special_activity_2023_score,
           max(tmp.teacher_app_score) as teacher_app_score,
           max(tmp.student_app_score) as student_app_score
    from (
...
--数据汇总
INSERT OVERWRITE  TABLE ads__nation__province_stat__d PARTITION (dt='${biz_date}')
select
    '${biz_date}' as stat_date,
    t1.province_id,
    t1.province_name,
    (t2.teacher_register_score + t2.student_register_score + t2.guardian_register_score) as register_score,
    t2.teacher_register_score,
    t2.student_register_score,
    t2.guardian_register_score,
    (t2.area_activation_score + t2.school_activation_score + t2.teacher_verify_score + t2.area_manager_active_score) as manage_score,
    t2.area_activation_score,
    t2.school_activation_score,
    t2.teacher_verify_score,
    t2.area_manager_active_score,
    (t2.area_pv_score + t2.register_pv_score + t2.resource_pv_score + t2.resource_assessment_score) as resource_view_score,
    t2.area_pv_score,
    t2.register_pv_score,
    t2.resource_pv_score,
    t2.resource_assessment_score,
    (t2.class_group_score + t2.user_participate_score + t2.activity_publish_score) as group_score,
    t2.class_group_score,
    t2.user_participate_score,
    t2.activity_publish_score,
    (t2.summer_special_activity_2022_score + t2.summer_special_activity_2023_score + t2.winter_special_activity_2023_score) as special_activity_score,
    t2.summer_special_activity_2022_score,
    t2.summer_special_activity_2023_score,
    t2.winter_special_activity_2023_score,
    (t2.teacher_app_score + t2.student_app_score) as app_score,
    t2.teacher_app_score,
    t2.student_app_score,
    current_timestamp as stat_time
from (select province_id, province_name from nation_province_data group by province_id, province_name) t1
inner join final_all_data t2 on t1.province_id = t2.province_id