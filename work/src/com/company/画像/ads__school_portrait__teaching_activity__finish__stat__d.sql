-- ==================================================================
-- 脚本名称： ads__school_portrait__teaching_activity__finish__stat__d
-- 所属专题： 学校画像
-- 实现功能： 学校画像--教学活动完成情况
-- 任务流：学校画像（提问、家校共育、活动、教研活动）
-- 统计周期： 日
-- 创建者：   10017103
-- 创建日期： 2023-12-29
-- 修改者：
   ...
   --发布的活动信息
    publish_activity_info AS (
        SELECT DISTINCT activity_id
                      ...
                      , vi.group_school_id                  AS school_id
                      , vi.group_id
         ...
         --通知、成绩管理、课程表触达就算完成
          SELECT user_id,activity_original_id,activity_type
                    ...
                    WHERE dt = '${full_biz_date}'
                      ...
                      AND activity_deleted = 0
                      AND activity_original_id <> '-1'
                      AND activity_type in('score','notice','course-schedule')
         ) vuas
...
--收到活动
   ...
   , yesterday_stat_data AS (
    SELECT stat_date
         ...
         , finish_rate_section
         , stat_time
    FROM ads__school_portrait__teaching_activity__finish__stat__d a
    WHERE dt = DATE_SUB('${biz_date}', 1)
      AND period_type_code <> (99)
      AND a.period_code NOT IN (SELECT b.period_code
                                FROM all_data b
                                GROUP BY b.period_code)
)
INSERT OVERWRITE TABLE ads__school_portrait__teaching_activity__finish__stat__d PARTITION (dt = '${biz_date}')
SELECT stat_date
         , period_code
         , period_type_code
         , period_start_date
